...
def load_data(file_name):
    ...
    if "ascii" in file_name:
        values = pd.read_csv(
            file_name, sep="\t", skiprows=32, index_col=False, na_values="NaN"
        )
    elif "tab" in file_name:
        values = pd.read_csv(
            file_name, sep="\t", skiprows=26, index_col=False, na_values="NaN"
        )
    return values
...
def rename_columns(values, file_name):
    if "ascii" in file_name:
        ...
        values = values.rename(
            columns={
                "Date/Time local (local time (UTC+08))": "timestamp",
...
def select_time_period(values, period):
    ...
    if len(period) == 2:
        return values.loc[
            values["timestamp"].dt.month.between(period[0], period[1]), values.columns
        ]
    else:
        return values[
            (values["timestamp"] >= "2017-07-01 00:00:00")
            & (values["timestamp"] <= "2017-07-15 23:50:00")
        ]
...
def calculate_rad_force(values):
    ...
    values["radForce"] = (
        values["SWdn[W m-2]"] - values["SWup[W m-2]"] + values["LWdn[W m-2]"]
    )
    return values
