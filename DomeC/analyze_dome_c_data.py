...
def find_data_in_u_bin(data, bin_mean=5.5, bin_var=1.5):
    if "U2[m s-1]" in data.columns:
        data.loc[
            (data["U2[m s-1]"] < (bin_mean - bin_var))
            | (data["U2[m s-1]"] > (bin_mean + bin_var))
        ] = np.nan
    else:
        data.drop(
            data[
                (data["U_9m [m/s]"] < (bin_mean - bin_var))
                | (data["U_9m [m/s]"] > (bin_mean + bin_var))
            ].index,
            inplace=True,
        )
...
def get_yearly_statistics(data):
    ...
    for cur_year in data["year"].unique():
        ...
        statistics[cur_year] = (
            np.round(calculate_mean(cur_year_data), 2),
...
def get_Dome_C_statistics(year=2017):
    ...
    data_in_u_bin_4_7 = find_data_in_u_bin(
        data_season.copy(), bin_mean=5.5, bin_var=1.5
    )
