...
@dataclass
class Parameters:
    Q_i: float = 70.0  # isothermal net radiation [W/m^2]
    Lambda: float = 7.0  # lumped parameter [W/m^2K]
    kappa: float = 0.4  # von Karman constant [-]
    cv: float = 1000  # heat capacity of the soil per surface area [J/m^2K]
    zr: float = 40.0  # reference height [m]
    Tr: float = 285.0  # temperature at reference height [K]
    rho: float = 1.2  # density of air at constant pressure [kg/m^3]
    cp: float = 1005.0  # heat capacity of air at constant pressure [J/kg^3K]
    grav: float = 9.81  # gravitational constant
    z0: float = 0.03  # roughness length [m]
    ...
    t_end_h: float = 24 * 1.0  # simulation end time [hours]
    ...
    delta_T_0: float = 10  # initial condition for delta T
