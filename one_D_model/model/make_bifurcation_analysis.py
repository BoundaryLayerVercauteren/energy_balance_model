...
configure_plotting_style(
    ...
    medium_size=medium_font_size,
    large_size=large_font_size,
)
...
def setup_for_equilibrium_analysis(param):
    ...
    if param.stab_func_type == "short_tail":
        DSargs.fnspecs = {
            "f": (
                ["x"],
                "exp(-2 * alpha * z_r * g/T_r * x/U**2 - (alpha * z_r * g/T_r * x/U**2)**2)",
            )
        }
    ...
    DSargs.varspecs = {
        "x": "Q_i - Lambda*x - rho * c_p * (k/log(z_r/z_0))**2 * U * x * f(x)",
        "w": "x-w",
    }
...
def plot_solution_trajectories(args, param):
    ...
    plt.savefig(
        param.sol_directory_path + "sol_trajectories.pdf", bbox_inches="tight", dpi=300
    )
...
def make_bifurcation_diagram(DSargs, ax, title, data_val=None, save_values=False):
    ...
    PCargs.MaxNumPoints = (
        150  # The following 3 parameters are set after trial-and-error
    )
    ...
    try:
        ...
        bif_point_1 = [
            PC["EQ1"].getSpecialPoint("LP1")["U"],
            PC["EQ1"].getSpecialPoint("LP1")["x"],
        ]
        bif_point_2 = [
            PC["EQ1"].getSpecialPoint("LP2")["U"],
            PC["EQ1"].getSpecialPoint("LP2")["x"],
        ]
        ...
        stable_points_idx = np.sort(
            np.concatenate(
                (
                    np.where(u_values == bif_point_1[0]),
                    np.where(u_values == bif_point_2[0]),
                )
            ).flatten()
        )
        ...
        ax.plot(
            u_values[: stable_points_idx[0]],
            delta_T_values[: stable_points_idx[0]],
            color=color[0],
        )
        ax.plot(
            u_values[stable_points_idx[1] :],
            delta_T_values[stable_points_idx[1] :],
            color=color[0],
        )
        # Plot unstable equilibria over wind forcing
        ax.plot(
            u_values[stable_points_idx[0] : stable_points_idx[1]],
            delta_T_values[stable_points_idx[0] : stable_points_idx[1]],
            linestyle="--",
            color=color[0],
        )
        # Mark bifurcation points
        ax.plot(
            bif_point_1[0], bif_point_1[1], marker="o", markersize=5, color=color[0]
        )
        ax.plot(
            bif_point_2[0], bif_point_2[1], marker="o", markersize=5, color=color[0]
        )
    ...
    if data_val is not None:
        ...
        xbox1 = TextArea(
            r"$U$, ",
            textprops=dict(
                color=color[0], size=medium_font_size, ha="left", va="bottom"
            ),
        )
        xbox2 = TextArea(
            r"$U_{8m}$ ",
            textprops=dict(
                color=color[2], size=medium_font_size, ha="left", va="bottom"
            ),
        )
        xbox3 = TextArea(
            "[$\mathrm{ms^{-1}}$]",
            textprops=dict(
                color="black", size=medium_font_size, ha="left", va="bottom"
            ),
        )
...
def make_bifurcation_analysis(params, data=None, save_values=False):
    ...
    plt.savefig(
        ...
        + "bifurcation_diagram_single_"
        + params.stab_func_type
        + ".png",
        bbox_inches="tight",
        dpi=300,
    )
