...
def solve_randomized_model(params):
    ...
    with multiprocessing.Pool(processes=params.num_proc) as pool:
        for idx, res in enumerate(
                pool.imap_unordered(
                    partial(solve_SDEs_wrapper, func_name=solve_SDEs.solve_SDE_with_internal_var, param=params),
                    range(params.num_simulation))):
            np.save(params.sol_directory_path + 'temporary/SDE_internal_var_sol_delta_T_' + str(idx) + '.npy', res)

    # Combine solution files into a single one and delete single files
    combine_npy_files(params.num_simulation, params.sol_directory_path, 'SDE_internal_var_sol_delta_T')
...
def solve_model_with_randomized_parameter(params, function_name, sol_file_name):
    ...
    with multiprocessing.Pool(processes=params.num_proc) as pool:
        for idx, res in enumerate(pool.imap_unordered(
                partial(solve_SDEs_wrapper, func_name=function_name, param=params), range(params.num_simulation))):
            if np.shape(res)[1] > 1:
                np.save(params.sol_directory_path + 'temporary/' + sol_file_name + '_delta_T_' + str(idx) + '.npy',
                        res[0][:])
                np.save(params.sol_directory_path + 'temporary/' + sol_file_name + '_param_' + str(idx) + '.npy',
                        res[1][:])
            else:
                np.save(params.sol_directory_path + 'temporary/' + sol_file_name + '_delta_T_' + str(idx) + '.npy',
                        res.flatten())
