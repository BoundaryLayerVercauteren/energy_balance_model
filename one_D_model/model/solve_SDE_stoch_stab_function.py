...
def define_SDE_with_time_dependent_u(t, delta_T, f_stab, param):
    c_D = calculate_neutral_drag_coefficient(param)
    ...
    u = param.u_range[idx]
    return (1 / param.cv) * (
            param.Q_i - param.Lambda * delta_T - param.rho * param.cp * c_D * u * delta_T * f_stab)
...
def solve_SDE_with_stoch_stab_function_multi_noise_time_dependent_u(param):
    ...
    # Define functions for 2D SDE
    def _f(X, t):
        return np.array([define_SDE_with_time_dependent_u(t, X[0], X[1], param),
                         param.relax_phi * (X[1] - calculate_stability_function(param, X[0], param.U))])
