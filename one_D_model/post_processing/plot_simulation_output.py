...
make_distribution_plot(
    SDE_sol_delta_T.flatten(),
    params,
    "distribution_" + file_name_delta_T,
    r"$\Delta T$ [K]",
)
...
def plot_potentials_and_output_distribution(param_class, delta_T_data, file_str):
    ...
    if param_copy.stab_func_type == "short_tail":
        ...
        for idx, u_elem in enumerate(u_list_st):
            potential_st = solve_ODE.calculate_potential(
                delta_T_range, u_elem, param_copy
            )
            (p1,) = ax1.plot(
                delta_T_range,
                -potential_st,
        ...
        for idx, u_elem in enumerate(u_list_lt):
            potential_lt = solve_ODE.calculate_potential(
                delta_T_range, u_elem, param_copy
            )
            (p3,) = ax1.plot(
                delta_T_range,
    ...
    plt.savefig(
        f"{param_class.sol_directory_path}{file_str}_dist_potentials_{param_copy.stab_func_type}.pdf",
        bbox_inches="tight",
        dpi=300,
    )


plot_potentials_and_output_distribution(
    params, SDE_sol_delta_T.flatten(), file_name_delta_T
)
    ...
    for idx in vis_idx:
        if sim_type == sim_type_options[0]:
            plot_output.make_line_plot_of_single_solution(
                params,
                params.t_span_h,
                SDE_sol_delta_T[idx][:].flatten(),
                "SDE_sol_delta_T_over_time_sim" + str(idx) + ".pdf",
                xlabel="t [h]",
                ylabel=r"$\Delta T$ [K]",
            )
        else:
            ...
            plot_output.make_line_plot_of_solution_and_parameter(
                params,
                params.t_span_h,
                np.array([SDE_sol_delta_T[idx, :], SDE_sol_param[idx, :]]).T,
                line_labels,
                f"{file_name_delta_T}_over_time_sim{idx}.pdf",
                xlabel="t [h]",
                ylabel=y_axes_label_1,
