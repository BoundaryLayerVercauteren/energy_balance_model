...
def get_minimal_sigma_with_trans_for_every_u(trans_statistics, u_range):
    if 'internal_variability' in output_directory or 'stability_function' in output_directory:
        ...
        for u in u_range:
            ...
            if trans_statistics[idx_first_sigma_with_enough_trans][3] < trans_percentage:
        ...
        for idx_u, u in enumerate(u_range):
            for idx_sigma_u, sigma_u_val in enumerate(sigma_u):
                ...
                if trans_statistics[idx_first_sigma_with_enough_trans][3] < trans_percentage:
                    ...
                    first_sigma_with_enough_trans[idx_u, idx_sigma_u] = \
...
det_equilibria = np.loadtxt('output/sensitivity_study/equilibria_det_model/x_values.txt', delimiter=',',
                            dtype=float).flatten()
det_equilibria_u = np.loadtxt('output/sensitivity_study/equilibria_det_model/u_values.txt', delimiter=',',
                              dtype=float).flatten()
...
ax2.plot(det_equilibria_u, det_equilibria, color='red', label='equilibria det. model', linestyle='--', alpha=0.5)
...
fig.legend(facecolor='white', edgecolor="black", frameon=True, bbox_to_anchor=(0.9, 0.6))
...
fig.tight_layout()
plt.savefig(f'{output_directory}transition_statistics_{trans_percentage}_{subset_data_size}_{index}.pdf',
